...
var useAddressValidatorStopAsking = "0";
...
function addressValidatorContinue(type, currentAddress) {

    if (useAddressValidatorStopAsking == "1") {
        setCookie(`addressValidator-${type}`, currentAddress, 1);
    }    
...
function check_address2_pos(type) {
    if (jQuery("#hdnAddrressValidatorResult").val() == "1") {
        ...
        if (_3dThemeType != 'core') {
            ...
            jQuery('#divAddrressValidator').modal({ containerCss: { borderWidth: "1px" }, backdrop: 'static', keyboard: false });
        }
    }
}
...
function select_shipping(shipid) {
    ...
    if (!parseInt(jQuery('#wid').val()) > 0) {
        if ((shipid + "").indexOf("instorepickup") > -1) {
            params = params + '&location_' + shipid + '=' + jQuery('#location_' + shipid).val().replace(/ /g, '+');
            ...
            if (jQuery('#shipping_firstname').val() == jQuery('#checkout2_in-store-pickup-header').val() && jQuery('#shipping_firstname').val() != undefined) {
    ...
    jQuery.ajax({
        ...
        data: params,
        cache: false,
        async: typeof (bol_async_ship_ajax) != "undefined" ? bol_async_ship_ajax : true,
        success: function (strResult) {
            ...
            if (jQuery("#pp_message").length > 0 && jQuery("#hdnBalance").length > 0) {
                jQuery("#pp_message").attr("data-pp-amount", jQuery("#hdnBalance").val())
            }
        },
...
function doPayment() {
    ...
    jQuery.ajax({
        ...
        success: function (strResult) {
            ...
            var bolSameContent = false;
            if (typeof jQuery.parseHTML == "function") {
                ...
                if (strResult.indexOf("paypal_express.asp?cancel=1") > 0) {
                    ...
                    if (radioCollectionOriginal.length == radioCollectionNew.length && bolPayPalAdvSelected == false) {
            ...
            if (!bolSameContent) {
                ...
                //if (strResult.indexOf("hdnPayFortID") > 0) {
                //GW removed from DB (v8.2)
                //}

                if (strResult.indexOf("hdnBreadCheckoutID") > 0) {
                    if (jQuery("#divBreadCheckoutSection").html() != jQuery("#divBreadCheckoutSection", strResult).html())
                        jQuery("#divBreadCheckoutSection").html(jQuery("#divBreadCheckoutSection", strResult).html());
                }

                if (strResult.indexOf("hdnKlarnaPaymentsID") > 0) {
                    if (jQuery("#divKlarnaSection").html() != jQuery("#divKlarnaSection", strResult).html())
                        jQuery("#divKlarnaSection").html(jQuery("#divKlarnaSection", strResult).html());
                }
            }
...
function doCheckout(objForm, extraParams) {
    ...
    if (objForm.onsubmit !== undefined && objForm.onsubmit !== null && !objForm.onsubmit()) {
        return false;
    }

    if (!checkotherreqfields())
        return false;
    else {
        isSubmitComplete = false;
        bolCheckSubmitted_validation = true;
    }    
    ...
    //PayPal Checkout
    ////////////////////////////////////////////////////////////////////////////////////
    if (jQuery("#hdnPPCAdvCCid").length > 0 && jQuery("#hdnPPCAdvCCid").val() == jQuery('input[name=payment]:checked').val() && extraParams == "checkoutButton") {
        return;
    }
    ////////////////////////////////////////////////////////////////////////////////////
    ...
    //Stripe New
    //////////////////////////////////////////////////////////////////////////////////
    var stripeCheckoutType = isStripeNewCheckout();
    if (stripeCheckoutType == 'card' || stripeCheckoutType == "wid") {
        if (extraParams == "checkoutButton") {
            bolCheckSubmitted_validation = false;
            doStripeCheckoutNew(objForm, stripeCheckoutType, '', '');
            return;
        }
    }
    //////////////////////////////////////////////////////////////////////////////////
    ...
    //////////////////////////////////////////////////////////////////////////////////
    if (jQuery("#hdnKlarnaPaymentsID").length > 0 && typeof idSelected !== 'undefined' && idSelected.indexOf(jQuery("#hdnKlarnaPaymentsID").val() + '---KP') >= 0) {
        if (extraParams == "checkoutButton") {
    ...
    //Shift4Payments
    /////////////////////////////////////////////////////////////////////////////////////
    if (jQuery("#hdnShift4i4GoID").length > 0 && jQuery("#hdnShift4i4GoID").val() == jQuery('input[name=payment]:checked', '#' + objForm.name).val()) {
        if (extraParams == "checkoutButton") {
            if (typeof objI4Go === "object") {
                objI4Go.submit();
            }
            return;
        }
...
function doCheckoutNormal(objForm, strExtraParam) {
    ...
    if ((objForm.onsubmit !== undefined && objForm.onsubmit !== null && !objForm.onsubmit()) || !checkotherreqfields()) {
    ...
    for (var prop in formArray) {
        if (prop.indexOf('Paymetric') < 0) {
            ...
            if (bolAuth_opaquedata == true && prop.indexOf('_ocardno') > 0 && prop.indexOf('_ocardnotoken') < 0) {
    ...
    jQuery.ajax({
        ...
        success: function (strResult) {
            ...
            //Stripe New action
            ////////////////////////////////////////////////////////////////////////////////////
            if (isStripeNewCheckout() != '') {
                if (strResult.startsWith("{")) {
                    var jsonResult = JSON.parse(strResult);
                    handleAction(objForm, jsonResult);
                    return;
                }
            }
            ////////////////////////////////////////////////////////////////////////////////////
...
function checkoutSuccess(objResponse) {
    ...
    if (((objResponse.responseText != undefined) && objResponse.responseText == "run_paymetric" || objResponse.responseText.indexOf("klarna_session_updated:true") > 0))
        return;

    if (objResponse.responseText != undefined && objResponse.responseText.substring(0, 4) == 'ZIP|')
        return;    
    ...
    if (objResponse.responseText != undefined && isStripeNewCheckout() != '' && objResponse.responseText.startsWith('{')) {
    ...
    if (divCheckout.indexOf('initI4goTrueTokenObj') >= 0) {
        var end = divCheckout.indexOf('initI4goTrueTokenObj');
        if (end >= 0)
            alert(divCheckout.slice(0, end));
        return void (0);
    }
...
function initpage() {
    ...
    if (strOriginalDivPaymentContent.indexOf("onVmeReady") > 0)
        strOriginalDivPaymentContent = "";
    var radioCollectionOriginal;
    if (typeof jQuery.parseHTML == "function") {
        radioCollectionOriginal = jQuery("input:radio", jQuery.parseHTML(strOriginalDivPaymentContent));
    }
...
function check_address2(type) {
    ...
    if (((szip.length >= 1 && sstate.length >= 1 && scountry.length >= 1) || (scountry != 'US' && scountry.length >= 1)) && addrchange == 1 && ajax_request_progress == 0) {
        ...
        var currentAddress = `${jQuery("#" + type + "_address").pVal()}-${jQuery("#" + type + "_address2").pVal()}-${jQuery("#" + type + "_city").pVal()}-${jQuery("#" + type + "_state").pVal()}-${jQuery("#" + type + "_zip").pVal()}-${type}`;

        if (useAddressValidator == "1" && myCountry == "US" && currentAddress != getCookie(`addressValidator-${type}`)) {
        ...
        jQuery.ajax({
            ...
            success: function (strResult) {
                ...
                try {
                    if(jQuery("#shipResult")[0] == undefined){
                        select_shipping(0)
                    }
                } catch (e) {
                    //
            ...
            if (obj1 != undefined)
                if (id == '') {
                    if (divs[x].id.indexOf("---KP---") > 1)
                        break;
                    else if (jQuery(obj1).find('#hdnPPCAdvCCid').length < 1) {
                        obj1.style.display = '';
                        break;
                    }
                }
    ...
    jQuery.ajax({
        ...
        success: function (jsonResult) {
            ...
            if (jsonResult.Success) {
                ...
                var errormsg = "Error getting FedEx locations from Fedex Location Service";

                if (jsonResult.Message !== undefined && jsonResult.Message !== "undefined" && jsonResult.Message !== "")
                    errormsg = errormsg + ". " + jsonResult.Message;

                jQuery('#status').html(errormsg);
                jQuery('#status').show();

                if (jsonResult.Code !== undefined && jsonResult.Code !== "undefined" && jsonResult.Code !== "")
                    errormsg = errormsg + ". (Error Code: " + jsonResult.Code + ")";
...
function isStripeNewCheckout() {
    if (jQuery("#hdnStripeNewJS_ID").length > 0) {
        if (jQuery("#hdnStripeNewJS_ID").val() == jQuery('input[name=payment]:checked').val())
            return 'card';
        else if (jQuery("#wid").length > 0 && jQuery('#wid').val() == 4)
            return 'wid';
        else if (jQuery("#authCIMProfileID").length > 0 && jQuery("#authCIMProfileID").val().indexOf('_pm|||') > 0 && jQuery('input[name=payment]:checked').val() == 'online-CIM')
            return 'cim';
    }
    return '';
}
...
if (typeof _3dThemeType == 'undefined') var _3dThemeType = 'html5';
